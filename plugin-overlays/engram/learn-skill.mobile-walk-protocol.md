...
**VERIFY, everything else** — threshold, `node_kind: "procedure"`, `transfer_ready`, lapsed, `effectively_relearn`, or assigned to a running experiment arm: **a composed chain, a step assembly, or a real production.** A threshold concept is exactly what recognition flatters, so a chain of picks is still refused here; an arm item is stashed for the blind assessor by design. This carve-out is the same triage `/review`'s checkpoint protocol runs, applied one beat earlier.
...
**The recognition floor for a carved-out node** — this is a deliberate widening, and it is priced. Where no composed chain, assembly or production is available, a carved-out VERIFY **may** be served as a pairing, a partition, or a find-the-flaw, rather than not served at all. The alternative on this surface is not free recall; it is nothing, and a threshold node that is never checked away from the desk is not being protected by the stricter rule, only skipped. But such an item is rated **`hard` at best, never `good`**, it carries `--source mobile-recognition` so the record can find every one of them, and the node stays provisional exactly as before. A learner who wants the cap lifted speaks or types, or Solidifies at the desk.
