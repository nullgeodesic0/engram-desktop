...
**1. Pace data is thin and lumpy, which sets the whole tone of exam mode.** Measured across all four real topics: `grad-classical-mechanics` has 18 encodes over **8 distinct active days** spanning Jul 12–23; `long-form-humanities` has 5 encodes on a **single day**; `applied-policy` has 2 encodes over 2 days. A "you've averaged N/day" figure computed over this is unstable enough to be misleading — a learner who did one good session and nothing since would read either "5/day" or "0.4/day" depending entirely on the denominator. The spec therefore requires the denominator to be stated, and requires a minimum before any pace figure is shown at all.
