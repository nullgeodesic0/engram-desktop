...
server.registerTool(
  'emit_card_pack',
  {
    ...
    description:
      ...
      "Beats run in grammar order and each appears at most once. SELF_EXPLAIN and a carved-out VERIFY (threshold, procedure, transfer-ready, lapsed, or an experiment arm) may never be a plain menu — use ladder, cloze, compose or recall there. At a carved-out VERIFY specifically, match/sort/flaw are ALSO admitted as a priced fallback when nothing production-grade fits: the app rates that item at hard-at-best and stamps it separately, precisely so it never reads as a clean check. Reach for it only when a ladder or a composed chain genuinely does not fit the node — it is a floor under coverage, not a house style. A ladder's pool must hold at least twice its true steps, and every distractor must be competitive: a sign error, a right-step-wrong-order, a step from a neighbouring derivation, or the learner's own recorded misconception in their own words. Filler options make the card a coin flip and the receipt a lie.\n\n" +
      "A COMPOSE card is the phone's substitute for writing a derivation out. Its tokens are CLAUSES, not words: 'meet the employer as', 'cannot supply a theory of the whole society', '\\partial S/\\partial E'. Three or four lines, two or three clauses each, at most eight distinct tokens and at most twelve keys in the palette. Word-sized tokens turn the palette into a word search and bury the two keys that carry the claim among a dozen connectives — the learner should be choosing what the argument says, not reassembling its grammar.\n\n" +
